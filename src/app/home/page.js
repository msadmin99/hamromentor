...
function HomeContent() {
  ...
  return (
    <AppShell>
      <Header
        title="Dr. Gutka"
        subtitle={user ? `Welcome back, ${user.first_name || "student"}` : undefined}
        right={
          <>
            <BookmarkIcon />
            <SearchIcon />
      ...
      <div className="hm-page flex flex-col gap-5">
        ...
        {tests.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {tests.map((t) => (
                <Link key={t.id} href={`/tests/${t.id}`} className="hm-card flex items-center justify-between p-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-[var(--color-text)]">{t.title}</p>
                    <p className="text-xs text-[var(--color-text-muted)]">
                      {t.duration_minutes} mins · {t.question_count} MCQs
                    </p>
                  </div>
                  {t.is_pro && (
                    <span className="ml-2 flex-none rounded-md bg-brand-green-light px-2 py-1 text-[10px] font-bold text-brand-green">
                      PRO
                    </span>
                  )}
                </Link>
        ...
        <section className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
          <QuickLink href="/qbank" label="Question Bank" emoji="📚" />
          <QuickLink href="/mock-test" label="Mock Test" emoji="📝" />
          <QuickLink href="/daily-test" label="Daily Test" emoji="⏰" />
          <QuickLink href="/grand-test" label="Grand Test" emoji="🏆" />
          <QuickLink href="/past-year-questions" label="Past Year Qs" emoji="📖" />
          <QuickLink href="/videos" label="Videos" emoji="🎬" />
        </section>
      </div>
    </AppShell>
  );
...
function QuickLink({ href, label, emoji }) {
  return (
    <Link href={href} className="hm-card flex flex-col items-center gap-1.5 py-4">
